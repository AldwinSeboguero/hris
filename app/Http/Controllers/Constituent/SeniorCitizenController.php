...
class SeniorCitizenController extends Controller
{
    public function index(){   
        ...
        return Inertia::render('Constituent/SeniorCitizen/Index',[
            'barangays' =>  Barangay::orderBy('id')->get(),
            // 'sitios' =>  Sitio::orderBy('id')->get(),
            'max_age' => Constituent::orderBy('birthdate')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [60] )
             ->whereHas(
                'characteristic', function ($q) {
                    $q->where('pwd', true);
                }
            )->withCount('pwds')->having('pwds_count','>',0)->first()->age,
            'pensions' => $pensions,
            'seniorCitizens' => Constituent::orderBy('birthdate')
            ->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [60] ) ->whereHas(
                'characteristic', function ($q) {
                    $q->where('pwd', true);
                }
            )->withCount('pwds')->having('pwds_count','>',0)->paginate()->total(),
            'seniorCitizensMale' => Constituent::orderBy('birthdate')
            ->where('sex','male')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [60] )
             ->whereHas(
                'characteristic', function ($q) {
                    $q->where('pwd', true);
                }
            ...
            ->where('sex','female')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [60] )
             ->whereHas(
                'characteristic', function ($q) {
                    $q->where('pwd', true);
                }
            ...
            ->whereNull('sex')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [60] )
             ->whereHas(
                'characteristic', function ($q) {
                    $q->where('pwd', true);
                }
