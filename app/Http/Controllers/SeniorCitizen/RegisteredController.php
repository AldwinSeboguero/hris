...
class RegisteredController extends Controller
{
    public function index(){   
        ...
        return Inertia::render('SeniorCitizens/Registered/Index',[
            'barangays' =>  Barangay::orderBy('id')->get(),
            // 'sitios' =>  Sitio::orderBy('id')->get(),
            'max_age' => Constituent::orderBy('birthdate')->first()->age,
            'pensions' => $pensions,
            ...
            ->where('sex','male')->withCount('pwds')->having('pwds_count','>',0)->paginate()->total(),
            'seniorCitizensFemale' => Constituent::orderBy('birthdate')
            ->where('sex','female')->withCount('pwds')->having('pwds_count','>',0)->paginate()->total(),
            'seniorCitizensUnknown' => Constituent::orderBy('birthdate')
            ->whereNull('sex')->withCount('pwds')->having('pwds_count','>',0)->paginate()->total(),
        ]);
    }
    public function show(RS $request)
    {
        ...
        ->when(
            $disability && $disability != ['blank'], function ($query) use ($disability) {
                $query->whereHas(
                    'disabilities', function ($q) use ($disability) {
                        $q->whereIn('disability_type_id', $disability);
                    }
                );
            }
        )
        ->when(
            $disability == ['blank'], function ($query) use ($disability) {
                $query->withCount('disabilities')->having('disabilities_count',0);
            }
        )
        ->when(
