...
class ConstituentController extends Controller
{
    public function index(){
        return Inertia::render('Constituent59/Index',[
            ...
            'pensions' => PensionType::orderBy('id')->get(),
            'seniorCitizens' => Constituent::orderBy('birthdate')
            ->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [59] )->paginate()->total(),
            'seniorCitizensMale' => Constituent::orderBy('birthdate')
            ->where('sex','male')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [59] )->paginate()->total(),
            'seniorCitizensFemale' => Constituent::orderBy('birthdate')
            ->where('sex','female')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [59] )->paginate()->total(),
            'seniorCitizensUnknown' => Constituent::orderBy('birthdate')
            ->whereNull('sex')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [59] )->paginate()->total(),
        ]);
    }
    public function show(RS $request)
    {
        ...
        $constituents = new SeniorCitizenCollection(Constituent::orderByDesc('birthdate')    
        ->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [59] )
        ->when(
    ...
    public function pdf(RS $request){
        ...
        ->get()->map(function($inner) use($search,$age,$request,$barangay,$sx,$birthday,$pension,$annualincome,$min,$max){
            $constituents = Constituent::orderByDesc('birthdate')
            ->whereHas(
            ...
            ->when($min==null && $max==null, function ($query) use ($min,$max) {
                $query->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [59] );
                 }
        ...
        $pdf = PDF::loadView('pdf/aged-59-above-senior-citizen', compact(
            'date',
            'sitios'
        ))->setPaper('a4', 'landscape');
        // if($sitio){
        
        return $pdf->stream('aged-59-above-senior-citizen.pdf');
        // }
        // else{
        //     return response()->json([
    ...
    public function constituentsCount(RS $request){
