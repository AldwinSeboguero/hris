...
class ChidrenController extends Controller
{
    public function index(){   
        ...
        return Inertia::render('Constituent/Children/Index',[
            'barangays' =>  Barangay::orderBy('id')->get(),
            // 'sitios' =>  Sitio::orderBy('id')->get(),
            'max_age' => Constituent::orderBy('birthdate') ->whereHas(
                'characteristic', function ($q) {
                    $q->where('pwd', true);
                }
            )->whereRaw( 'timestampdiff(year, birthdate, curdate()) between ? and ?', [5,17] )->first()->age,
            'pensions' => $pensions,
            ...
            )->withCount('pwds')->having('pwds_count','>',0)->whereRaw( 'timestampdiff(year, birthdate, curdate()) between ? and ?', [5,17] )->paginate()->total(),
    ...
    public function show(RS $request)
    {
        ...
        $constituents = new SeniorCitizenCollection(Constituent::orderByDesc('updated_at')->orderBy('sex')->orderBy('birthdate')    
         ->whereHas(
            ...
            )->withCount('pwds')->having('pwds_count','>',0)->whereRaw( 'timestampdiff(year, birthdate, curdate()) between ? and ?', [5,17] )
        ->when(
