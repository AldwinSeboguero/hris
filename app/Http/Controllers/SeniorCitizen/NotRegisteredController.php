...
class NotRegisteredController extends Controller
{
    public function index(){
       
        return Inertia::render('SeniorCitizens/NotRegistered/Index',[
            'barangays' =>  Barangay::orderBy('id')->get(),
            // 'sitios' =>  Sitio::orderBy('id')->get(),
            Constituent::orderBy('birthdate') ->whereHas(
    ...
    public function show(RS $request)
    {
        ...
        $constituents = new SeniorCitizenCollection(Constituent::orderBy('updated_at')->orderBy('sex')->orderBy('birthdate')    
        ...
        ->when(
            $min != 0, function ($query) use ($min,$max) {
    ...
    public function pdf(RS $request){
        ...
        $pdf = PDF::loadView('pdf/not-registered-senior-citizen', compact(
            'date',
            'sitios'
        ...
        return $pdf->stream('not-registered-senior-citizen.pdf');
        // }
        // else{
        //     return response()->json([
    ...
    public function notregisteredSeniorCount(RS $request){
