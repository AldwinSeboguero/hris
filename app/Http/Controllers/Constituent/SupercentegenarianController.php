...
class SupercentegenarianController extends Controller
{
    public function index(){   
        ...
        return Inertia::render('Constituent/Supercentenarian/Index',[
            'barangays' =>  Barangay::orderBy('id')->get(),
            // 'sitios' =>  Sitio::orderBy('id')->get(),
            'max_age' => Constituent::orderBy('birthdate')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [110] )->first()->age,
            'pensions' => $pensions,
            'seniorCitizens' => Constituent::orderBy('birthdate')
            ->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [110] )->paginate()->total(),
            'seniorCitizensMale' => Constituent::orderBy('birthdate')
            ->where('sex','male')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [110] )->paginate()->total(),
            'seniorCitizensFemale' => Constituent::orderBy('birthdate')
            ->where('sex','female')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [110] )->paginate()->total(),
            'seniorCitizensUnknown' => Constituent::orderBy('birthdate')
            ->whereNull('sex')->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [110] )->paginate()->total(),
        ]);
    }
    public function show(RS $request)
    {
        ...
        ->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [110] )
        ->when(
    ...
    public function pdf(RS $request){
        ...
        ->get()->map(function($inner) use($search,$age,$request,$barangay,$sx,$birthday,$pension,$annualincome,$min,$max){
            ...
            ->when($min==null && $max==null, function ($query) use ($min,$max) {
                $query->whereRaw( 'timestampdiff(year, birthdate, curdate()) >= ?', [110] );
                 }
